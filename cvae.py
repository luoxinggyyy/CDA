...
class DeltaTVAE(BaseSynthesizer):
    ...
    def fit(self, train_data, discrete_columns=()):
        ...
        for i in range(self.epochs):
            ...
            for id_, data in enumerate(self.loader):
                ...
                if self._verbose:
                    print(f'Epoch {i + 1}, Loss 1: {loss_1.detach().cpu(): .4f},'  # noqa: T001
                          f'Loss 2: {loss_2.detach().cpu(): .4f}',
                          flush=True)
                epoch_loss1 += loss_1.detach().cpu().item()
                epoch_loss2 += loss_2.detach().cpu().item()
                num_batches += 1
                        # 计算epoch平均损失
            ...
            # 记录损失历史
            self.train_loss1_history.append(avg_loss1)
            self.train_loss2_history.append(avg_loss2)
            self.train_total_loss_history.append(total_loss)
            print("Training Loss History:")
            print("Loss1 (Reconstruction):", self.train_loss1_history)
            print("Loss2 (KL Divergence):", self.train_loss2_history)
            print("Total Loss:", self.train_total_loss_history)
