...
def smote_augmentations(x_train, y_train, generator_type,
                       oversample_num=10000, seed=42,
                       knn_neighbors=3, categorical_features=[]):
    """
    多分类smote增强
    """
    ...
    # 计算每个类别的比例
    class_ratios = {cls: np.sum(y_train == cls) / y_train.size for cls in classes}
    # 计算每个类别的过采样数量
    class_num = {cls: int(oversample_num * class_ratios[cls]) for cls in classes}

    method = getattr(sys.modules[__name__], generator_type)
    kwargs = {
        'random_state': seed,
        'sampling_strategy': class_num
    }
    if "SMOTENC" in generator_type:
        kwargs['categorical_features'] = categorical_features
    if "SMOTETomek" == generator_type or "SMOTEENN" == generator_type:
